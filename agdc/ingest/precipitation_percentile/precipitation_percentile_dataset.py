...
from .precipitation_percentile_bandstack import PrecipitationPercentileBandstack
import re
...
class PrecipitationPercentileDataset(AbstractDataset):
    """Dataset class for moisture percentile datasets."""
    ...
    def __init__(self, dataset_path):
        ...
        self._satellite_tag = "Percentile"
        self._satellite_sensor = "Precipitation"

        command = "find %s -name '*precipitation_percentile*' | sort" % dataset_path
        result = execute(command)['stdout'].split('\n')
        self._dataset_path = os.path.abspath(dataset_path)


        self._precipitation_percentile_85_band_file=result[0]
        self._precipitation_percentile_95_band_file=result[1]
        self._precipitation_percentile_99_band_file=result[2]

        fileName, fileExtension = os.path.splitext(self._precipitation_percentile_85_band_file)

        self._ds = gdal.Open(self._precipitation_percentile_85_band_file, gdal.GA_ReadOnly)

        if not self._ds:
            raise DatasetError("Unable to open %s" % self.get_dataset_path())

        self._dataset_size = (os.path.getsize(self._precipitation_percentile_85_band_file) +
                              os.path.getsize(self._precipitation_percentile_95_band_file) +
                              os.path.getsize(self._precipitation_percentile_99_band_file))/1000
        ...
        #self._cloud_cover_percentage = 0 #radar
        #LOGGER.debug('CloudCover = %f', self._cloud_cover_percentage)


        self._xml_text=""

        self._completion_datetime = completionDate
    ...
    def stack_bands(self, band_dict):
        ...
        bands into tiles.
        """
        return PrecipitationPercentileBandstack(self, band_dict)
