...
from .moisture_percentile_bandstack import MoisturePercentileBandstack
import re
...
class MoisturePercentileDataset(AbstractDataset):
    ...
    def __init__(self, dataset_path):
        ...
        self._satellite_sensor = "Moisture"

        command = "find %s -name '*moisture_percentile*' | sort" % dataset_path
        result = execute(command)['stdout'].split('\n')
        self._dataset_path = os.path.abspath(dataset_path)


        self._moisture_percentile_65_band_file=result[0]
        self._moisture_percentile_75_band_file=result[1]
        self._moisture_percentile_85_band_file=result[2]

        fileName, fileExtension = os.path.splitext(self._moisture_percentile_65_band_file)

        self._ds = gdal.Open(self._moisture_percentile_65_band_file, gdal.GA_ReadOnly)

        if not self._ds:
            raise DatasetError("Unable to open %s" % self.get_dataset_path())

        self._dataset_size = (os.path.getsize(self._moisture_percentile_65_band_file) +
                              os.path.getsize(self._moisture_percentile_75_band_file) +
                              os.path.getsize(self._moisture_percentile_85_band_file))/1000
        ...
        self._mtl_text = None
        self._processor_level = "Moisture_Percentile"
    ...
    def stack_bands(self, band_dict):
        ...
        bands into tiles.
        """
        return MoisturePercentileBandstack(self, band_dict)
